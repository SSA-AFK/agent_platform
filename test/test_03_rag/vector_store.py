...
from test.test_03_rag.utils.file_handler import listdir_with_allowed_type, txt_loader, pdf_loader, get_file_md5_hex
from test.test_03_rag.utils.path_tool import get_abs_path

logger = get_logger(name="vector_store")

#失败后查询重写，次数为3，若都失败则向用户反馈

#混合检索

#多类别数据库文件：本地用chroma（pdf+txt），线上用milvus（图片检索）
...
class VectorStoreService:
    ...
    def get_all_documents(self) -> List[Document]:
        ...
        if self._all_documents is None:
            # 从向量库加载全量文档（不同向量库API略有差异）
            # Chroma示例：
            all_docs = self.vector_store.get()  # 返回dict: {ids: [...], documents: [...], metadatas: [...]}
            # 处理空值情况
        ...
        for path in allowed_files_path:
            ...
            try:
                ...
                for i in range(0, len(split_document), batch_size):
                    ...
                    logger.debug(f"插入批次 {i//batch_size + 1}/{(len(split_document) + batch_size - 1)//batch_size}")
